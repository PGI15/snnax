...
class SNN(eqx.Module):
    ...
    def __init__(self, 
        ...
        conn,inp,out = snn.composed.gen_feed_forward_struct(18)
        self.ro_int = ro_int
        self.burnin = burnin
        graph = snn.GraphStructure(18, inp,out,conn)
        ...
        layers=[
            ...
            eqx.nn.LayerNorm(8*size_factor, eps=1e-5, elementwise_affine=False),
...
def create_functions(model, optim, filter_spec):
    ...
    def compute_loss(model, x, y, rng_key):
        ...
        y1h = jax.nn.one_hot(y, num_classes=11)
...
if __name__ == "__main__":
    ...
    import wandb
    wandb.init(project="debug", config=args['config']) 
    ...
    dataloader_train, _ = create_dataloader.create_dataloader(                                             
                                                                ...
                                                                target_transform_test  =  lambda x:x, drop_last = True)
    ...
    cell = snn.init.lsuv(model.cell, xs, init_key, var_tol=0.2, mean_tol=0.2, tgt_mean=tgt_mu, tgt_var=tgt_var, max_iters=100)
    ...
    compute_loss, make_step, accuracy = create_functions(model, optim, filter_spec)
